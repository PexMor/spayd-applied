...
function generateTextVersion(
    ...
    paymentDetailsList.forEach((payment, index) => {
        if (paymentDetailsList.length > 1) {
            text += `PAYMENT ${index + 1} OF ${paymentDetailsList.length}\n`;
            text += '-'.repeat(60) + '\n';
        } else {
        ...
        if (payment.dueDate) {
            ...
            text += `Due Date: ${date.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}\n`;
        }
        text += `Account: ${payment.iban}\n`;
        text += `Variable Symbol: ${payment.variableSymbol}\n`;
        if (payment.constantSymbol) {
            text += `Constant Symbol: ${payment.constantSymbol}\n`;
        }
        if (payment.staticSymbol) {
            text += `Specific Symbol: ${payment.staticSymbol}\n`;
        }
        text += `Message: ${payment.description}\n`;
        text += `\nScan the QR code in the HTML version to pay.\n`;
        text += '\n';
    });

    text += '═'.repeat(60) + '\n';
    text += 'Generated by SPAYD Applied';
...
export function generateEmailEml(
    ...
    const textVersion = generateTextVersion(body, paymentDetailsList, accountName);
...
export async function generateBatchZip(data: BatchData, config: BatchConfig) {
    if (!config.event) {
    ...
    for (let i = 0; i < data.rows.length; i++) {
        ...
        // Generate HTML
        const html = await generateEmailHtml(row, config, qrCodes, i);
